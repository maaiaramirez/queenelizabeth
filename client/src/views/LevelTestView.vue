...
onMounted(async () => {
  try {
    const existing = await fetchMyLastLevelTestResult()
    if (existing) {
...
<template>
  <div class="auth-page" style="max-width: 640px">
    ...
    <div v-else-if="stage === 'done' && result" style="text-align: center">
      <span style="font-size: 2rem; color: var(--gold)">🎓</span>
      <h2 style="font-family: var(--font-serif); color: var(--navy)">{{ alreadyDone ? 'Ya rendiste el Test de Nivel' : '¡Listo!' }}</h2>
      ...
      <p v-if="alreadyDone" style="color: var(--text-muted); font-size: 0.85rem; margin-bottom: 1.5rem">
        El Test de Nivel se rinde una sola vez por alumno.
      </p>
